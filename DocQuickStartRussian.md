...
_С исключением частей, относящихся к самой установке QMapShack данное резюме также действительно для не-Windows 
операционных систем._

QMapShack для Windows (вкратце QMS) является 64-разрядным приложением и требует поэтому 64-разрядной операционной системы Windows.

Это резюме довольно коротко, чтобы  быстро привести пользователя к работе с QMS. Многие из описанных
действия могут быть выполнены по-другому. Обсуждение этих дополнительных возможностей подавляется.
...
* _QMapShack_:
    ...
    * [Загрузить QMS от сервера Bitbucket](https://bitbucket.org/maproom/qmapshack/downloads "Загрузить QMS файл начальной установки").
...
* В окне `Выбор компонентов` выберите `MSVC ++ 2013 SP1 runtime` (это может быть опущен, если
...
* _Необязательно:_ Откройте файл `Onlinemaps.zip` и копируйте файлы, содержащиеся в нем в папку `Карты`.
...
* _Необязательно:_ Откройте файл `N59E030.hgt.zip` или `О36.zip` и копируйте файлы, содержащиеся в нем в папку `DEM`.
...
* Графический интерфейс содержит некоторые подсказки о рекомендуемых первых действий. Они описаны в следующих пунктах. Похожая
  информация часто получается во всплывающей справке, если мышь переместить на объект показанный на экране.
...
QMS предоставляет различные формы хранения данных. Рекомендуемая и мощная форма есть хранение в
...
QMS базы данных являются подходящим средством для строения четкой древовидной структуры данных похоже на древовидную структуру каталогов.
...
* Установив крючок в поле перед именем папки, папки открываются в окне проекта и данные, содержащиеся в ней (если также выбраны) 
  показаны в окне карт. Для более четкого определения источника проекта (папки) имя родидельской папки добавляется
  к названию папки за сепаратором "__@__". Эта дополнительная информация не добавляется к папкам верхнего уровня. Описанная 
  процедура позволяет целенаправленное и структурированное отображение выбранных данных в проекте и в окне карт.
...
* Повторите предыдущий шаг пока конец трека не достигнута.
...
* В окно базы данных появляется новую папку в используемой базе данных.
