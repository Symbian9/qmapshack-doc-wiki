...
[^4]:
    To be precise, `make check` checks all _tracked_ files in the wiki
    repository which are _not marked for removal_ and all new files
    _marked for addition_ for being referenced in at least one
    _toplevel_ `*.md` file.  Besides it checks all local _Markdown_
    links **without** a fragment identifier in all _toplevel_ `*.md`
    files for pointing to _tracked_ files _not marked for removal_ or to
    new files _marked for addition_, and all local _Markdown_ links
    **with** a fragment identifier in all _toplevel_ `*.md` files for
    being defined in some _toplevel_ `*.md` file.  This implies that
    `make check` can neither find undefined _Markdown_ links in `*.md`
    files still residing in directory `playground/` nor correctly check
    local _Markdown_ links **without** a fragment identifier pointing to
    _untracked_ files not (yet) _marked for addition_.
