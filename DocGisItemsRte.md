...
There is also a ready to use collection of database files for [download](https://jeepxj.de/pub/routino/). **Windows: Due to limitations in Microsoft's POSIX API Routino can't handle files larger than 4GB. Europe exceeds that size. You have to build your own smaller database with less countries.** 

To use the database files from the server you have to un-zip the files into a directory and QMapShack's Routino setup to register the path. Click the blue folder to do so.
